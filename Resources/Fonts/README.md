# Bundled font notices

These fonts are distributed under their embedded license terms. The SIL Open Font License 1.1 text is in `OFL.txt`; Cascadia Code's license is in `CascadiaCode-LICENSE.txt`.

- Cascadia Code — © 2021 Microsoft Corporation. All Rights Reserved.
- Caveat — Copyright 2014 The Caveat Project Authors.
- Comic Neue — Copyright 2014 The Comic Neue Project Authors.
- Inconsolata — Copyright 2006 The Inconsolata Project Authors.
- Nunito — Copyright 2014 The Nunito Project Authors.
- Virgil — Copyright © 2011 Your Own Font Foundry. All rights reserved.
